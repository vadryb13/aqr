...
- **Fallback-режим без LLM**: пайплайн работает без API-ключей (регекспный планировщик, шаблонный нарратор, отключённый reviewer)
- **LLM-точки**: `ChatPlanner` (голь-→план), `InsightReviewer` (топ-5 + детерминистичные инсайты → 1-3 доп. наблюдения), `Narrator` (весь результат → русский отчёт). Все через `litellm`, все с fallback
- **24 теста**: 12 на валидацию, 5 e2e на пайплайн, 7 на InsightReviewer
